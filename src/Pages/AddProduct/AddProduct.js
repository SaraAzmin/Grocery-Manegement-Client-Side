...
const AddProduct = () => {
  ...
  const onSubmit = (data) => {
    const url = "https://grocery-management-server-side.vercel.app/product";
    fetch(url, {
      method: "POST",
      headers: {
  ...
  return (
    <div className="mt-10">
      <h3 className="text-3xl text-green-600 font-semibold pb-5 mb-5 border-b-2 border-green-600 inline-block uppercase">
        Add New Product
      </h3>
        ...
        <div className="flex flex-col justify-center items-center mb-5">
          <form onSubmit={handleSubmit(onSubmit)}>
            ...
            <input
              ...
              className="text-gray-900 text-lg font-normal hidden"
              {...register("email", { required: true })}
            />
            <input
              type="submit"
              className=" inline-block px-6 py-3 bg-green-600 text-white font-medium text-sm leading-tight uppercase rounded shadow-md hover:bg-green-800 hover:shadow-lg ml-3"
              value="Add Product"
            ></input>
          </form>
        </div>
